...
RSpec.describe Hyrax::Transactions::Steps::ApplyWorkflowOnAdminSetChange do
  ...
  def single_state_workflow(name, states:)
    ...
    { workflows: [{ name:, label: name, description: name, actions: }] }
  end
  ...
  def transferred_work(original_admin_set:, new_admin_set:)
    work = FactoryBot.valkyrie_create(:generic_work_resource, :with_admin_set,
  ...
  context 'when admin set has not changed' do
    let(:work) { FactoryBot.valkyrie_create(:generic_work_resource, depositor: depositor.user_key) }
  ...
  context 'when admin set changed and both admin sets are workflow-managed' do
    let(:work) { transferred_work(original_admin_set: old_admin_set, new_admin_set:) }
  ...
  context 'when transferring between two workflow-managed admin sets with different managers' do
    ...
    def grant_manage_responsibilities(workflow:, user:)
      ...
      Hyrax::Workflow::PermissionGenerator.call(agents: user, roles: magic_roles, workflow:)
    end
    ...
    it 'flips actionable roles from the old admin set manager to the new admin set manager' do
      entity = Sipity::Entity.find_by(proxy_for_global_id: Hyrax::GlobalID(work).to_s)
      expect(actions_for(user: user_one, entity:)).to be_any
      expect(actions_for(user: user_two, entity:)).to be_none

      step.call(work)

      entity.reload
      expect(actions_for(user: user_one, entity:)).to be_none
      expect(actions_for(user: user_two, entity:)).to be_any
    end
  end
  ...
  context 'when the work has no Sipity::Entity yet' do
    let(:work) do
      ...
      w = FactoryBot.valkyrie_create(:generic_work_resource, :with_admin_set,
