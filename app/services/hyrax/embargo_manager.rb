...
module Hyrax
  ...
  class EmbargoManager # rubocop:disable Metrics/ClassLength
    ...
    class << self
      ...
      def create_or_update_embargo_on_members(members, work)
        ...
        members.each do |member|
          ...
          Rails.logger.debug "⚠️⚠️⚠️ Work ID: #{work.id}"
          Rails.logger.debug "⚠️⚠️⚠️ Members count: #{members.count}"
          Rails.logger.debug "⚠️⚠️⚠️ Member ID: #{member.id}"
          Rails.logger.debug "⚠️⚠️⚠️ Work has embargo? #{work.embargo.present?}"
          Rails.logger.debug "⚠️⚠️⚠️ Member has embargo? #{member.embargo.present?}"
          Rails.logger.debug "⚠️⚠️⚠️ Work embargo updated at: #{work.embargo&.updated_at}"
          Rails.logger.debug "⚠️⚠️⚠️ Member embargo updated at: #{member.embargo&.updated_at}"
          ...
          if member.embargo && member_embargo_needs_updating
            ...
            Rails.logger.debug "⚠️⚠️⚠️ Embargo release date updated to: #{member.embargo.embargo_release_date}"
            Rails.logger.debug "⚠️⚠️⚠️ Member embargo visibility during embargo updated to: #{member.embargo.visibility_during_embargo}"
            Rails.logger.debug "⚠️⚠️⚠️ Member embargo visibility after embargo updated to: #{member.embargo.visibility_after_embargo}"
          else
            Rails.logger.debug "⚠️⚠️⚠️ No Member embargo found, copying from Work."
            work_embargo_manager = Hyrax::EmbargoManager.new(resource: work)
            Rails.logger.debug "⚠️⚠️⚠️ Work embargo release date: #{work.embargo.embargo_release_date}"
    ...
    def copy_embargo_to(target:)
      Rails.logger.debug "⚠️⚠️⚠️ From #copy_embargo_to: Under embargo?: #{under_embargo?}"
      return false unless under_embargo?

      target.embargo = Hyrax.persister.save(resource: Embargo.new(clone_attributes))
      Rails.logger.debug "⚠️⚠️⚠️ From #copy_embargo_to: New embargo on target: #{target.embargo.inspect}"
